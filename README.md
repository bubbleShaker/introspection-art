...
| 帯域 | 範囲 | 効かせる先 |
| --- | --- | --- |
| 低 | 20–160 Hz | 波の高さ、波紋の発生、リングの脈 |
| 中 | 160–2000 Hz | 月の大きさ、雲の照り、光の溢れ |
| 高 | 2–8 kHz | 星の瞬き、水面のきらめきの速さ |

帯域とは別に、**時間領域の波形**（音そのものの形）も読んでいる。
帯域は「どれくらい強いか」しか持たないので、手前に浮かぶ白いリングの
輪郭はこちらから作る。波形を円周に巻きつけ、`半径 = 基準半径 × (1 + 波形)`
とする。左右に折り返して対称にしているのは、音の波形が輪になっていないため
...
手前の白いリングだけは、この三段が終わったあとに p5 の既定の描き方へ戻して
線で引いている。線 1 本のために板とテクスチャを一往復させる理由が無いため。
滲みの段より後ろなので、リングに bloom は乗らない。白の芯とにじみは、
太く薄い線と細く濃い線の二度描きで作っている。
...
```sh
npm test    # 純粋な計算部分（帯域の切り出し、波形の畳み込み、打点検出、波紋の減衰）
npm run build
```
