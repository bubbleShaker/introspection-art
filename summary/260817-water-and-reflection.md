...
### 「あとから来た答え」に現在を上書きさせてはいけない（#5）

同梱音源が在るかの確認は通信なので、いつ返るか分からない。その間にユーザーが
自分の音源を渡していることがあり、あとから来た確認結果でそれを上書きすると、
**再生中なのに操作ボタンが消える**（確認が「無い」だった場合）か、
**同梱曲に差し替わる**（「在る」だった場合）。

音源の出所（`trackFrom`）を持たせ、ユーザーが渡したものは奪わないようにした。
非同期の答えを状態に書き戻す時は、書く直前に「まだ自分の番か」を確かめる。
同じ考えで、続けて音源を渡した時の `play()` にも採り込み番号を振っている。

## 音源をどう渡すか

三つの口がある。いずれも `adoptFile()` に集まる。

| 口 | 使える場面 |
| --- | --- |
| `public/audio/introspection.mp3` | 置いてあれば起動時に自動で読む |
| 画面へドラッグ&ドロップ | PC のみ |
| 下部の「音源を選ぶ」 | どこでも。スマホはこれ |

`<input type="file">` を `hidden` ではなく visually-hidden で隠しているのは、
`display: none` の input への `click()` を無視する実装があるため。
レイアウト上は在るまま、目にだけ入らないようにしている。
